...
def days_to_birthday(birthday: datetime):
    if birthday is None:
        return None
    this_day = datetime.today()
    birthday_day = datetime(this_day.year, birthday.month, birthday.day)
    if birthday_day < this_day:
        birthday_day = datetime(this_day.year + 1, birthday.month, birthday.day)
...
def add_contact(*args):
    ...
    if birthday is None and email == '' and address == '':
        dml.insert_adressbook(name.value, phone.value, birthday, email, address)
    if birthday is not None and email == '' and address == '':
        dml.insert_adressbook(name.value, phone.value, datetime.strftime(birthday.value, '%Y-%m-%d'), email, address)
    if birthday is not None and email != '' and address == '':
        dml.insert_adressbook(name.value, phone.value, datetime.strftime(birthday.value, '%Y-%m-%d'), email.value, address)
...
def add_birthday(*args):
    ...
    dml.add_birthday(name.value, datetime.strftime(birthday.value, '%Y-%m-%d'))


@InputError
def days_to_user_birthday(*args):
    name = Name(args[0])
    birthday = dml.days_to_user_birthday(name.value)
    if birthday is None:
        print('User there is no birthday')
    else:
